...
class TestPlatforms(unittest.TestCase):
    def setUp(self):
        ...
        self.modules_patch = patch.dict(
            "sys.modules", {"config": self.config, "logging": self.logging_mock, "time": Mock()}
        )
    ...
    def test_000_text_chat_successful(self):
        ...
        _builtins["print"].assert_has_calls(
            (call("Hi"), call("Can I help you with anything?"), call("Exiting..."))
        )
    ...
    def test_001_text_chat_respond_exception(self):
        ...
        gz_chat_mock = Mock(name="godzillops.Chat")
        gz_chat_mock().respond = Mock(
            side_effect=[resp_generator(("Huh?",)), resp_generator((ValueError("BOOM"),))]
        )
        _builtins = {"print": Mock(), "input": Mock(side_effect=["Hey", "Bye", EOFError("FIN")])}
        ...
        _builtins["print"].assert_has_calls(
            (
                call("Huh?"),
                call("An error occurred - check the logs. Reinitializing GZ."),
                call("Exiting..."),
            )
        )
    ...
    def test_002_slack_chat_successful(self):
        ...
        slack_events = [
            [{"type": "message", "text": "What?", "channel": self.channel, "user": self.user_id}],
            [
                {
                    "type": "message",
                    "text": "Huh?",
        ...
        gz_chat_mock().respond.assert_has_calls(
            (call("What?", context=context), call("Okay?", context=context))
        )
        post_message_mock.assert_has_calls(
            (
                call(text="Huh?", channel=self.channel, as_user=True),
                call(
                    text="Admin Command completed.", channel=im_open["channel"]["id"], as_user=True
                ),
            )
        )
    ...
    def test_005_slack_get_user_info_dies(self):
        ...
        post_message_mock = Mock(name="chat.postMessage")
        slack_mock().api_call = _slack_api_call_builder(
            {"users.info": user_info_mock, "chat.postMessage": post_message_mock}
        )
        gz_chat_mock = Mock(name="godzillops.Chat")
